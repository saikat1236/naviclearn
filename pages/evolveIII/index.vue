<template>
  <b-container fluid class="level1 bg">
    <h1 class="section-header white" style="padding: 2rem 0 1rem">
      EVOLVE &nbsp;- &nbsp; Level III
    </h1>
    <b-row class="">
      <b-col lg="8">
        <b-row>
          <b-col lg="5">
            <ul class="listleftprep">
              <li class="listitemgo">
                <b-icon icon="check-circle" variant="info"></b-icon> Batch limit
                -10 students
              </li>
              <li class="listitemgo">
                <b-icon icon="check-circle" variant="info"></b-icon> 10 Week
                Courses
              </li>
              <li class="listitemgo">
                <b-icon icon="check-circle" variant="info"></b-icon> 5
                practicals
              </li>
            </ul>
          </b-col>
          <b-col lg="6">
            <ul class="listleftprep">
              <li class="listitemgo">
                <b-icon icon="check-circle" variant="info"></b-icon> Live
                Interactive Sessions
              </li>
              <li class="listitemgo">
                <b-icon icon="check-circle" variant="info"></b-icon> Segregated
                Q&A sessions
              </li>
              <li class="listitemgo">
                <b-icon icon="check-circle" variant="info"></b-icon> 1-to-1
                feedback
              </li>
            </ul>
          </b-col>
        </b-row>
        <b-row class="courseboxleft">
          <h3 class="middlehead">ABOUT</h3>
          <p class="preptext">
            Dominate<br />
            Expand your horizons and become the leader in your peer group with
            level 3. With a curated selection of 10 pro subjects, this course
            allows you to perfect your role and career specialization. Own
            it.<br />
            EVOLVE-III is created for marketers by marketers. In a rapidly
            changing communications industry, EVOLVE aims to work with limited
            batches and grow your understanding about the future of marketing.
            Whether you come from Advertising, Digital Marketing, Public
            Relations or social media marketing, EVOLVE will add value to your
            personal marketing journey.
            <br />
            <b>*Recommended to take the courses in sequence. </b>
          </p>
          <!-- <p class="coursetiming">
            Next Batch: &nbsp;<b-icon icon="calendar3" variant="info"></b-icon>
            Starting 1st August 2020
            <b-icon icon="clock-history" variant="info"></b-icon> 6:30 to 7:30
            PM, IST.
          </p> -->
        </b-row>
      </b-col>
      <b-col lg="4" class="coursebar">
        <div class="seatsleft">
          <h2 class="navic-color">10/10 SEATS LEFT</h2>
          <b-progress
            height="1.5 rem"
            width="2rem"
            :value="20"
            variant="info"
            striped
            :animated="animate"
            class="mt-2"
          ></b-progress>
          <br />
          <br />
          <h4>STARTS IN</h4>
          <b-row class="inlineflexer justify-content-around">
              <!-- <b-col lg="1" sm="1" class="seatbutton">{{displayday}}</b-col
              ><b-col lg="1" sm="1" class="seatbutton">{{displayhour}}</b-col
              ><b-col lg="1" sm="1" class="seatbutton">{{displaymin}}</b-col> -->
              <b-col lg="1" sm="1" class="seatbutton">0</b-col
              ><b-col lg="1" sm="1" class="seatbutton">0</b-col
              ><b-col lg="1" sm="1" class="seatbutton">0</b-col>
            </b-row>
            <b-row class="inlineflexer justify-content-around">
              <b-col>DAYS</b-col>
              <b-col >HOURS</b-col>
              <b-col  >MINS</b-col>
            </b-row>
          <div class="py-5">
            <h2 class="navic-color pb-4">₹34990 +<h5 style="display: inline"> 18% GST</h5></h2>
            <!-- <a href="/registration"><b-button class="btn-black"> ENROLL NOW </b-button></a> -->
          </div>
          <form><script src="https://checkout.razorpay.com/v1/payment-button.js" data-payment_button_id="pl_K0MpvY7pr5S4oo" async> </script> </form>
        </div>
      </b-col>
    </b-row>
    <h2 class="center centerhead py-5">Modules in This Course</h2>
    <b-row align-self="center">
      <b-col align-self="start"></b-col>
      <b-col lg="6" align-self="center">
        <div class="accordion accordian-body" role="tablist">
          <b-row class="modulesbody">
            <b-col cols="2" class="modulesleft">
              <img
                src="~/assets/evolve3/ai.svg"
                class="moduleimg"
                height="55px;"
              />
            </b-col>
            <b-col cols="10" class="py=0">
              <b-card no-body class="mb-1 darkbody">
                <b-card-header header-tag="header" class="p-1" role="tab">
                  <b-button
                    class="text-left"
                    block
                    v-b-toggle.accordion-1
                    variant="dark"
                    >Ai In Marketing 2</b-button
                  >
                </b-card-header>
                <b-collapse
                  id="accordion-1"
                  visible
                  accordion="my-accordion"
                  role="tabpanel"
                >
                  <b-card-body>
                    <b-card-text
                      ><p class="white insidetext">
                        &#9679;&nbsp;The AI revolution<br />
                        &#9679;&nbsp;The Turing Test<br />
                        &#9679;&nbsp;How AI communicates<br />
                        &#9679;&nbsp;How communicators use AI<br />
                        &#9679;&nbsp;Case 1<br />
                        &#9679;&nbsp;Case 2<br />
                        &#9679;&nbsp;How can you use AI?<br />
                        &#9679;&nbsp;Future of AI
                      </p></b-card-text
                    >
                  </b-card-body>
                </b-collapse>
              </b-card>
            </b-col>
          </b-row>
          <b-row class="modulesbody">
            <b-col cols="2" class="modulesleft">
              <img
                src="~/assets/evolve3/content.svg"
                class="moduleimg"
                height="55px;"
              />
            </b-col>
            <b-col cols="10" class="py=0">
              <b-card no-body class="mb-1 darkbody">
                <b-card-header header-tag="header" class="p-1" role="tab">
                  <b-button
                    class="text-left"
                    block
                    v-b-toggle.accordion-2
                    variant="dark"
                    >Content Marketing</b-button
                  >
                </b-card-header>
                <b-collapse
                  id="accordion-2"
                  visible
                  accordion="my-accordion"
                  role="tabpanel"
                >
                  <b-card-body>
                    <b-card-text
                      ><p class="white insidetext">
                        &#9679;&nbsp;What is content marketing?<br />
                        &#9679;&nbsp;How has it changed? Storytelling Automation
                        in Content<br />
                        &#9679;&nbsp;Change in content formats Old vs new<br />
                        &#9679;&nbsp;Use case 1<br />
                        &#9679;&nbsp;Use case 2
                      </p></b-card-text
                    >
                  </b-card-body>
                </b-collapse>
              </b-card>
            </b-col>
          </b-row>
          <b-row class="modulesbody">
            <b-col cols="2" class="modulesleft">
              <img
                src="~/assets/evolve3/digibrand.svg"
                class="moduleimg"
                height="55px;"
              />
            </b-col>
            <b-col cols="10" class="py=0">
              <b-card no-body class="mb-1 darkbody">
                <b-card-header header-tag="header" class="p-1" role="tab">
                  <b-button
                    class="text-left"
                    block
                    v-b-toggle.accordion-3
                    variant="dark"
                    >Digital Brand Design</b-button
                  >
                </b-card-header>
                <b-collapse
                  id="accordion-3"
                  visible
                  accordion="my-accordion"
                  role="tabpanel"
                >
                  <b-card-body>
                    <b-card-text
                      ><p class="white insidetext">
                        &#9679;&nbsp;Basics of brand design<br />
                        &#9679;&nbsp;What is a digital brand design?<br />
                        &#9679;&nbsp;Fundamentals of digital brand design<br />
                        &#9679;&nbsp;Digital consumer journey<br />
                        &#9679;&nbsp;Digital experience<br />
                        &#9679;&nbsp;Use-case of digital brand design
                      </p></b-card-text
                    >
                  </b-card-body>
                </b-collapse>
              </b-card>
            </b-col>
          </b-row>
          <b-row class="modulesbody">
            <b-col cols="2" class="modulesleft">
              <img
                src="~/assets/evolve3/digimarket.svg"
                class="moduleimg"
                height="55px;"
              />
            </b-col>
            <b-col cols="10" class="py=0">
              <b-card no-body class="mb-1 darkbody">
                <b-card-header header-tag="header" class="p-1" role="tab">
                  <b-button
                    class="text-left"
                    block
                    v-b-toggle.accordion-4
                    variant="dark"
                    >Digital Marketing - Tracking and Analysis</b-button
                  >
                </b-card-header>
                <b-collapse
                  id="accordion-4"
                  visible
                  accordion="my-accordion"
                  role="tabpanel"
                >
                  <b-card-body>
                    <b-card-text
                      ><p class="white insidetext">
                        &#9679;&nbsp;What is tracking and analysis?<br />
                        &#9679;&nbsp;Fundamentals of success<br />
                        &#9679;&nbsp;Expectation setting<br />
                        &#9679;&nbsp;Types of metrics<br />
                        &#9679;&nbsp;Tools to track marketing success<br />
                        &#9679;&nbsp;Use-case 1<br />
                        &#9679;&nbsp;Use-case 2
                      </p></b-card-text
                    >
                  </b-card-body>
                </b-collapse>
              </b-card>
            </b-col>
          </b-row>
          <b-row class="modulesbody">
            <b-col cols="2" class="modulesleft">
              <img
                src="~/assets/evolve3/game.svg"
                class="moduleimg"
                height="55px;"
              />
            </b-col>
            <b-col cols="10" class="py=0">
              <b-card no-body class="mb-1 darkbody">
                <b-card-header header-tag="header" class="p-1" role="tab">
                  <b-button
                    class="text-left"
                    block
                    v-b-toggle.accordion-5
                    variant="dark"
                    >Gamification</b-button
                  >
                </b-card-header>
                <b-collapse
                  id="accordion-5"
                  visible
                  accordion="my-accordion"
                  role="tabpanel"
                >
                  <b-card-body>
                    <b-card-text
                      ><p class="white insidetext">
                        &#9679;&nbsp;What is a game?<br />
                        &#9679;&nbsp;Gamification - Basics<br />
                        &#9679;&nbsp;The fundamental design of gamification<br />
                        &#9679;&nbsp;Gamification & Communications<br />
                        &#9679;&nbsp;Case 1<br />
                        &#9679;&nbsp;Case 2<br />
                        &#9679;&nbsp;Incentives
                      </p></b-card-text
                    >
                  </b-card-body>
                </b-collapse>
              </b-card>
            </b-col>
          </b-row>
          <b-row class="modulesbody">
            <b-col cols="2" class="modulesleft">
              <img
                src="~/assets/evolve3/group.svg"
                class="moduleimg"
                height="55px;"
              />
            </b-col>
            <b-col cols="10" class="py=0">
              <b-card no-body class="mb-1 darkbody">
                <b-card-header header-tag="header" class="p-1" role="tab">
                  <b-button
                    class="text-left"
                    block
                    v-b-toggle.accordion-6
                    variant="dark"
                    >Digital Communities and Groups</b-button
                  >
                </b-card-header>
                <b-collapse
                  id="accordion-6"
                  visible
                  accordion="my-accordion"
                  role="tabpanel"
                >
                  <b-card-body>
                    <b-card-text
                      ><p class="white insidetext">
                        &#9679;&nbsp;Who are the digital communities and
                        groups?<br />
                        &#9679;&nbsp;What is their role in social media
                        communication?<br />
                        &#9679;&nbsp;Types of groups and communities<br />
                        &#9679;&nbsp;Paid/Unpaid partnerships<br />
                        &#9679;&nbsp;Use-cases
                      </p></b-card-text
                    >
                  </b-card-body>
                </b-collapse>
              </b-card>
            </b-col>
          </b-row>
          <b-row class="modulesbody">
            <b-col cols="2" class="modulesleft">
              <img
                src="~/assets/evolve3/growth.svg"
                class="moduleimg"
                height="55px;"
              />
            </b-col>
            <b-col cols="10" class="py=0">
              <b-card no-body class="mb-1 darkbody">
                <b-card-header header-tag="header" class="p-1" role="tab">
                  <b-button
                    class="text-left"
                    block
                    v-b-toggle.accordion-7
                    variant="dark"
                    >Growth Hacking</b-button
                  >
                </b-card-header>
                <b-collapse
                  id="accordion-7"
                  visible
                  accordion="my-accordion"
                  role="tabpanel"
                >
                  <b-card-body>
                    <b-card-text
                      ><p class="white insidetext">
                        &#9679;&nbsp;What is Growth Hacking?<br />
                        &#9679;&nbsp;What is growth hacking on
                        social/digital?<br />
                        &#9679;&nbsp;Basic techniques of growth hacking<br />
                        &#9679;&nbsp;The role of JUGAAD<br />
                        &#9679;&nbsp;Use-case of growthhacking
                      </p></b-card-text
                    >
                  </b-card-body>
                </b-collapse>
              </b-card>
            </b-col>
          </b-row>
          <b-row class="modulesbody">
            <b-col cols="2" class="modulesleft">
              <img
                src="~/assets/evolve3/meme.svg"
                class="moduleimg"
                height="55px;"
              />
            </b-col>
            <b-col cols="10" class="py=0">
              <b-card no-body class="mb-1 darkbody">
                <b-card-header header-tag="header" class="p-1" role="tab">
                  <b-button
                    class="text-left"
                    block
                    v-b-toggle.accordion-8
                    variant="dark"
                    >Meme Marketing</b-button
                  >
                </b-card-header>
                <b-collapse
                  id="accordion-8"
                  visible
                  accordion="my-accordion"
                  role="tabpanel"
                >
                  <b-card-body>
                    <b-card-text
                      ><p class="white insidetext">
                        What is a meme?<br />
                        &#9679;&nbsp;History of memes<br />
                        &#9679;&nbsp;Types of memes<br />
                        &#9679;&nbsp;Use-cases of memes<br />
                        &#9679;&nbsp;Memes in India<br />
                        &#9679;&nbsp;How to create memes<br />
                        &#9679;&nbsp;Fundamentals of meme marketing
                      </p></b-card-text
                    >
                  </b-card-body>
                </b-collapse>
              </b-card>
            </b-col>
          </b-row>
          <b-row class="modulesbody">
            <b-col cols="2" class="modulesleft">
              <img
                src="~/assets/evolve3/meta.svg"
                class="moduleimg"
                height="55px;"
              />
            </b-col>
            <b-col cols="10" class="py=0">
              <b-card no-body class="mb-1 darkbody">
                <b-card-header header-tag="header" class="p-1" role="tab">
                  <b-button
                    class="text-left"
                    block
                    v-b-toggle.accordion-9
                    variant="dark"
                    >Metadata and keywords</b-button
                  >
                </b-card-header>
                <b-collapse
                  id="accordion-9"
                  visible
                  accordion="my-accordion"
                  role="tabpanel"
                >
                  <b-card-body>
                    <b-card-text
                      ><p class="white insidetext">
                        &#9679;&nbsp;What is metadata?<br />
                        &#9679;&nbsp;How does it work?<br />
                        &#9679;&nbsp;Use case of Metadata<br />
                        &#9679;&nbsp;What are keywords?<br />
                        &#9679;&nbsp;How do they work?<br />
                        &#9679;&nbsp;Use-case of keywords<br />
                        &#9679;&nbsp;Synergy
                      </p></b-card-text
                    >
                  </b-card-body>
                </b-collapse>
              </b-card>
            </b-col>
          </b-row>
          <b-row class="modulesbody">
            <b-col cols="2" class="modulesleft">
              <img
                src="~/assets/evolve3/regional.svg"
                class="moduleimg"
                height="55px;"
              />
            </b-col>
            <b-col cols="10" class="py=0">
              <b-card no-body class="mb-1 darkbody">
                <b-card-header header-tag="header" class="p-1" role="tab">
                  <b-button
                    class="text-left"
                    block
                    v-b-toggle.accordion-10
                    variant="dark"
                    >Regional Communications</b-button
                  >
                </b-card-header>
                <b-collapse
                  id="accordion-10"
                  visible
                  accordion="my-accordion"
                  role="tabpanel"
                >
                  <b-card-body>
                    <b-card-text
                      ><p class="white insidetext">
                        &#9679;&nbsp;What are regional communications?<br />
                        &#9679;&nbsp;What is the role of regional content in
                        India?<br />
                        &#9679;&nbsp;The role of social platforms in pushing
                        regional content<br />
                        &#9679;&nbsp;The situation today<br />
                        &#9679;&nbsp;Usecase 1<br />
                        &#9679;&nbsp;Usecase 2
                      </p></b-card-text
                    >
                  </b-card-body>
                </b-collapse>
              </b-card>
            </b-col>
          </b-row>
        </div>
      </b-col>
      <b-col align-self="end"></b-col>
    </b-row>
    <div class="coursebottom hidemobile">
      <b-row>
        <b-col lg="4" align-self="start"></b-col>
        <b-col lg="4" align-self="center" class="px-2">
          <div class="seatsleft">
            <h2 class="navic-color">10/10 SEATS LEFT</h2>
            <b-progress
              height="1.5 rem"
              width="2rem"
              :value="20"
              variant="info"
              striped
              :animated="animate"
              class="mt-2"
            ></b-progress>
            <br />
            <br />
            <h4>STARTS IN</h4>
            <b-row class="inlineflexer justify-content-around">
              <!-- <b-col lg="1" sm="1" class="seatbutton">{{displayday}}</b-col
              ><b-col lg="1" sm="1" class="seatbutton">{{displayhour}}</b-col
              ><b-col lg="1" sm="1" class="seatbutton">{{displaymin}}</b-col> -->
              <b-col lg="1" sm="1" class="seatbutton">0</b-col
              ><b-col lg="1" sm="1" class="seatbutton">0</b-col
              ><b-col lg="1" sm="1" class="seatbutton">0</b-col>
            </b-row>
            <b-row class="inlineflexer justify-content-around">
              <b-col>DAYS</b-col>
              <b-col >HOURS</b-col>
              <b-col  >MINS</b-col>
            </b-row>
            <div class="py-5">
              <h2 class="navic-color pb-4">₹34990 +<h5 style="display: inline"> 18% GST</h5></h2>
              <!-- <a href="/registration"><b-button class="btn-black"> ENROLL NOW </b-button></a> -->
              <!-- <form action="/student" method="POST">
<script
    src="https://checkout.razorpay.com/v1/checkout.js"
    data-key="rzp_test_enRiibCpGvB8WA"
    data-amount="3499000"
    data-currency="INR"
    data-buttontext=" ENROLL NOW "
    data-name="Acme Corp"
    data-description="A Wild Sheep Chase is the third novel by Japanese author Haruki Murakami"
    data-prefill.name="Gaurav Kumar"
    data-prefill.email="saikat1236@gmail.com"
    data-prefill.phone="7085959167"
    data-theme.color="#F37254"
></script>
<input type="hidden" custom="Hidden Element" name="hidden">
</form> -->
            </div>
          </div>
        </b-col>
        <b-col align-self="end"></b-col>
      </b-row>
    </div>
    <h2 class="center py-6">Who is EVOLVE-III made for?</h2>
    <div class="for center py-4">
      <b-img src="~/assets/buis.svg"></b-img>
      <b-img src="~/assets/pro.svg"></b-img>
      <b-img src="~/assets/stu.svg"></b-img>
    </div>
  </b-container>
</template>

<script>
export default {
  data () {
    return {
      // timerCount: 10
      displayday: 0,
      displayhour: 0,
      displaymin: 0,
      displaysec: 0,
      loaded: false,
      expired: false
    }
  },
  mounted () {
    this.showremaining()
  },
  computed: {
    _second: () => 1000,
    _minutes () {
      return this._second * 60
    },
    _hours () {
      return this._minutes * 60
    },
    _days () {
      return this._hours * 24
    }
  },
  methods: {
    formatNum: num => (num < 10 ? '0' + num : num),
    showremaining () {
      const timer = setInterval(() => {
        const now = new Date()
        const end = new Date(2022, 7, 17, 0, 0)
        const distance = end.getTime() - now.getTime()
        if (distance < 0) {
          clearInterval(timer)
          this.expired = true
        }
        const day = Math.floor(distance / this._days)
        const hour = Math.floor((distance % this._days) / this._hours)
        const min = Math.floor((distance % this._hours) / this._minutes)
        // const sec = Math.floor(distance / this._days)
        this.displayday = this.formatNum(day)
        this.displaymin = this.formatNum(min)
        this.displayhour = this.formatNum(hour)
        this.loaded = true
      }, 1000)
    }
  }
}
</script>

<style>
.seatsleft{
  font-family: NowThin;
font-style: normal;
font-weight: normal;
font-size: 14.4762px;
line-height: 14px;
text-align: center;
}
.level1{
  display: flexbox;
  color:white;
  padding:0px 100px;
}
.ul {
  list-style-type: none;
}
.listleftprep{
padding-inline-start: 0px !important;
font-family: NowThin;
font-style: normal;
font-weight: normal;
font-size: 16px;
line-height: 55px;
align-items: center;
}
.preptext{
  font-family: NowThin;
font-style: normal;
font-weight: normal;
font-size: 16px;
line-height: 26px;
/* or 162% */
text-align: justify;
letter-spacing: 0.0161em;
}
.middlehead{
  font-family: Brandon Grotesque;
font-style: normal;
font-weight: 420;
font-size: 24px;
line-height: 55px;
/* or 231% */
text-transform: uppercase;
color: #FFFFFF;
}
.coursebar{
  text-align: center;
}
.progress{
  border-radius: 20px;
}
.coursetiming{
  font-family: Now;
font-style: normal;
font-weight: 500;
font-size: 16px;
line-height: 22px;
}
.courseboxleft{
  text-align: left;
  padding: 10px 53px;
}
.coursebottom{
  text-align: center;
  padding: 20px;
}
.seatbutton{
background: #242728;
box-shadow: inset -4.8254px -4.8254px 4.8254px rgba(68, 68, 68, 0.25), inset 4.8254px 4.8254px 4.8254px rgba(2, 2, 2, 0.15);
border-radius: 48.254px;
margin: 22px 45px;
padding: 11px 13px 11px 13px;
display: inline-flex;
}
.coursecard{
  text-align: center;
  padding: 20px 20px;
  margin:20px;
}
.card-title{
  font-size:18px;
}
.card-img{
  height:100px;
}
.for{
  align-content: center;
}
.blackbar{
  background: #242728;
box-shadow: inset -4px -4px 4px rgba(68, 68, 68, 0.25), inset 4px 4px 4px rgba(2, 2, 2, 0.15);
border-radius: 10px;
display: flex;
text-align: center;
    padding:10px;
    margin: 15px;
}
.aquatext{
font-family: Now;
font-style: normal;
font-weight: bold;
font-size: 24px;
line-height: 26px;
}
.centrehead{
font-family: Brandon Grotesque;
font-style: normal;
font-weight: 900;
font-size: 24px;
line-height: 55px;
text-align: center;
}
</style>
